...
int Uniform_Cost_Search(vector<int> &queue)
{
...
int A_star_with_the_Misplaced_Tile(vector<int> &queue, int n)
{
    // we do not need to do anything for Uniform Cost Search
    ...
    for (int i = 0; i < queue.size(); i++)
    {
        misplaced = 0;
        anti_hash.clear();
        anti_hash_function(anti_hash, queue[i], n);
        ...
        paired.push_back(make_pair(misplaced, queue[i]));
    }
...
int A_star_with_the_Manhattan_distance(vector<int> &queue, int n)
{
    // we do not need to do anything for Uniform Cost Search
    vector<pair<int, int>> paired;
    vector<int> anti_hash;
...
void expand(vector<int> &queue, vector<int> &global_queue, int node, int n, int &expanded_size)
{
    ...
    for (int i = 0; i < anti_hash.size(); i++)
    {
        if (anti_hash.at(i) == 0)
        {
            ...
            if (0 <= north(x, y) && north(x, y) < anti_hash.size())
            {
                ...
                {
                    queue.push_back(hash_function(north_node));
                    global_queue.push_back(hash_function(north_node));
                    expanded_size++;
                }
                // cout << "north_node = { ";
                // for (int i : north_node)
                // {
                //     cout << i << ", ";
                // }
                // cout << "}; \n";
            ...
            if (0 <= south(x, y) && south(x, y) < anti_hash.size())
            {
                ...
                {
                    queue.push_back(hash_function(south_node));
                    global_queue.push_back(hash_function(south_node));
                    expanded_size++;
                }
                // cout << "south_node = { ";
                // for (int i : south_node)
                // {
                //     cout << i << ", ";
                // }
                // cout << "}; \n";
            ...
            if ((0 <= east(x, y) && east(x, y) < anti_hash.size()) && ((x + 1) < sqrt))
            {
                ...
                {
                    queue.push_back(hash_function(east_node));
            ...
            if ((0 <= west(x, y) && west(x, y) < anti_hash.size()) && (0 <= (x - 1)))
            {
                ...
                {
                    queue.push_back(hash_function(west_node));
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "t:n:m:")) != EOF)
        ...
        case 'm':
            ...
            break;
        case '?':
            fprintf(stderr, "usuage is \n ./program -t test t -n size of board -m method(0 is Uniform Cost Search, 1 is A* with the Misplaced Tile heuristic, "
                            "2 is A* with the Manhattan Distance heuristic)\n");
        default:
            cout << endl;
            abort();
        }
    ...
    if (t == 8)
    {
        ...
        shuffle(begin(board), end(board), re);
    }
    // depth 0
    if (t == 0)
    {
    ...
    while (queue.size() > 0)
    {
        ...
        temp_node = queue.at(0);
        // queue.erase(queue.begin());
        if (temp_node == goal_state)
        {
            printf("success! The maximum queue size is %d. The expanded nodes size is %d\n", queue_size, expanded_size);
            return 0;
        }
        expand(queue, global_queue, temp_node, n, expanded_size);
